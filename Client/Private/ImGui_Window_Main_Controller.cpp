...
void CImGui_Window_Main_Controller::Show_Window()
{
	...
	if (ImGui::Begin(m_pImGui_Manager->str_MainWindowType[m_pImGui_Manager->WINDOW_MAIN_CONTROLLER], NULL, window_flags))
	{
		...
		ImGui::SeparatorText("Windows");
		{
			...
			/* 메인 윈도우 트리거 버튼 */
			if (ImGui::Button(m_pImGui_Manager->str_MainWindowType[m_pImGui_Manager->WINDOW_MAIN_TRIGGER]))
			{
				m_pImGui_Manager->Set_Active_Main_Window(
					m_pImGui_Manager->WINDOW_MAIN_TRIGGER,
					!m_pImGui_Manager->m_pMainWindows[m_pImGui_Manager->WINDOW_MAIN_TRIGGER]->Is_Active());

			}
			ImGui::SameLine();

			/* 메인 윈도우 UI 버튼 */
			if (ImGui::Button(m_pImGui_Manager->str_MainWindowType[m_pImGui_Manager->WINDOW_MAIN_UI]))
			{
				m_pImGui_Manager->Set_Active_Main_Window(
					m_pImGui_Manager->WINDOW_MAIN_UI,
					!m_pImGui_Manager->m_pMainWindows[m_pImGui_Manager->WINDOW_MAIN_UI]->Is_Active());
