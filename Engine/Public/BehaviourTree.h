...
{
	/* Root : 자식 노드로 단 한개의 컴포짓(주로 시퀀스)을 갖는다.*/
	ROOT, 

	/* Composit : 여러 개의 자식 노드(Action, Composit, Decorator)를 가질 수 있다.*/
	...
	/* Decorator : 자식 노드가 언제(조건) 어떻게 실행될지를 결정한다. 단 하나의 자식 노드를 가지며 그 노드는 액션 노드이다.*/
	IF,
	WHILE,
	SUCCEEDER,
	FAILURER,
